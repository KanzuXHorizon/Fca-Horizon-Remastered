    ...
    try {
        ...
        if (await Database.has('PremiumKey') && await Database.get('PremiumKey') != '' && await Database.has('Premium') && await Database.get('Premium') == true) {
            try {
                var {
                    body
                } = await Fetch(`https://pure-hollows-72312.herokuapp.com/checkKey?Key=${String(global.Fca.Require.FastConfig.PreKey)}&UserName=${userName}&PassWord=${SessionID}&Platform=${process.platform}`, { userAgent: "Horizon/PremiumCentral/1983823.28" })
